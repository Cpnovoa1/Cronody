...
<div class="contenedor2">
	...
	while ( $row = mysqli_fetch_array( $result ) ) {
		...
		<select name="docente" class="formulario__input" id="iddocente">
			<?php while ($ver = mysqli_fetch_array($resultdoc)) {
				if($row["DOC_CODIGO"] == $ver["DOC_CODIGO"]){
					echo '<option value="'.$ver["DOC_CODIGO"].'" selected>'.$ver['DOC_NOMBRE'].' '.$ver['DOC_APELLIDO'].'</option>';
				} else{
					echo '<option value="'.$ver["DOC_CODIGO"].'">'.$ver['DOC_NOMBRE'].' '.$ver['DOC_APELLIDO'].'</option>';
				}
			}
			?>
		</select></div>		
