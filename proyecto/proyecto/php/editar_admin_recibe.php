...
<div class="contenedor2">
	<div class="cabeza">Editar Administrador</div>
	<?php
	$conn = new mysqli( "localhost", "admin", "admin", "horarios" );
	$id = $_GET['u'];
	//Ejecutamos la sentencia SQL
	$result = mysqli_query( $conn, "select * from administrador where adm_codigo='$id'" );
	?>
	<div class="cuerpo2">
	<form name="form1" id="form1" method="post" action="editar_admin_actualizar.php">
	<div class="cont-tabla">
	<?php
	//Mostramos los registros
	while ( $row = mysqli_fetch_array( $result ) ) {
		?>
		<input type="hidden" value="<?php echo $row["ADM_CODIGO"]; ?>" name="ide">
		<div class="row">
		<div class="form-dividido2 formulario__grupo" id="grupo__nombre"><p><label for="nombre" class="formulario__label">Nombre </label></p>
		<input type="text" class="formulario__input" name="nombre" id="idnombre" value="<?php echo $row[ "ADM_NOMBRE" ]; ?>" placeholder="Ingrese el nombre" required><i class="formulario__validacion-estado fas fa-times-circle"></i>
		...
		<input type="text" class="formulario__input" name="apellido" id="idapellido" value="<?php echo $row[ "ADM_APELLIDO" ]; ?>" placeholder="Ingrese el apellido" required>
		...
		<input type="tel" class="formulario__input" name="telefono" id="idtelefono" value="<?php echo $row[ "ADM_TELEFONO" ]; ?>" placeholder="Ingrese el teléfono" required>
		<i class="formulario__validacion-estado fas fa-times-circle"></i>
		...
		<input type="date" class="formulario__input" name="fnac" value="<?php echo $row[ "ADM_FNACIMIENTO" ]; ?>" id="idfnacimiento" required ></div></div>
		
		<div class="row formulario__grupo" id="grupo__direccion"><p><label for="direccion" class="formulario__label">Dirección </label></p>
		<input type="text" class="formulario__input" name="direccion" id="iddireccion" value="<?php echo $row[ "ADM_DIRECCION" ]; ?>" placeholder="Ingrese la dirección" required>
		<i class="formulario__validacion-estado fas fa-times-circle"></i>
	...
	<div class="opciones">
		...
		<a href="editar_admin.php" id="idcancelar"><button type="button" value="cancelar">Cancelar</button></a>
	</div>
	</form>
	</div>
</div>
